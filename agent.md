...
1. 接入方式：通过 LiteLLM 统一调用任意大模型（DeepSeek / Claude / GPT / NVIDIA NIM / Moonshot 等）。
2. LiteLLM 角色：作为唯一模型通道，将 OpenAI 格式请求转发到任意模型 API。
3. 凭据管理：API Key 通过 `config/model.yaml` 或环境变量注入，不写入仓库。
4. 稳定性：配置超时、重试、回退模型，避免单点失败导致主流程阻塞。
5. 成本控制：按任务类型选择模型档位，优先使用满足质量要求的最低成本模型。
6. 可观测性：记录 API 延迟、错误率、tokens 使用量，并纳入告警。
...
5.1 快速启动向导（可选能力包）：
- 运行 `make start` 可进入交互式向导（模型配置 + 启动）。
- 向导支持“一键启用 GitHub Agent 能力包”：GitHub MCP + 本机已安装 skills（`gh-address-comments`、`gh-fix-ci`、`playwright`、`gog`、`self-improving-agent`、`github-api-gateway`）。
- 未安装的 skill 仅提示缺失项，不会伪造能力可用状态；后续可在 `/dashboard/integrations` 启用。
- 可执行 `make install-skills` 将项目内 `skills/*` 链接到 `~/.codex/skills`，使本地 agent 可直接加载。
...
4. Phase D（已完成 - Agent 化重构）：
- 执行层全面 Agent 化：ExecutionAgent 使用 litellm.acompletion() + OpenAI Tool Calling ReAct 循环。
- PlannerService 不再按 source 硬编码步骤，改为输出单一 agent_execute 意图步骤。
- ToolRegistry 输出 OpenAI Function Calling JSON Schema 格式工具描述。
- 旧 Connector 逐步执行路径已移除，Agent 是唯一执行路径。
- 通过 LiteLLM 支持任意大模型（DeepSeek / Claude / GPT / NVIDIA NIM / Moonshot 等）。
...
## 执行框架（增量 v0.2 → v2.0 Agent 化重构）

在“感知 -> Context Space -> 候选任务”之后，执行侧已全面 Agent 化：

1. Perception Inbox：接收所有候选任务与候选动作。
2. Planner：将候选任务转为单一 `agent_execute` 步骤（LLM 推断意图）。
3. Conflict Arbiter：检测资源冲突与语义冲突，输出合并/串行/重规划/升级策略。
4. Policy Gate：基于风险、置信度、多源一致性与打扰预算进行执行判定。
5. ExecutionAgent（ReAct 循环）：
   - 通过 `litellm.acompletion()` 调用任意大模型。
   - 工具以 OpenAI Function Calling JSON Schema 格式传入。
   - 循环：推理 → 调用工具 → 获取结果 → 再推理 → ... 直到任务完成。
   - 最大轮次 `max_turns=15` 防止死循环。
6. ToolRegistry：统一工具注册中心，输出 OpenAI 格式 schema。
   - 内置工具：`get_repo_context`（仓库上下文）、`record_note`（记忆系统）。
   - MCP 元工具：将已启用的 MCP Server 包装为可调用工具。
   - Skill 元工具：将已启用的 Skill 包装为可调用工具。
