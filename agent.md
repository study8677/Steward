...
4. 启动本地依赖：
- 若需 Postgres：`docker compose up -d`。
- 初始化数据库：`make upgrade`。
...
- 该传感器会周期读取前台应用/窗口标题，并上报到 `/api/v1/webhooks/screen`。
- 可选环境变量：
...
- 首次运行若提示无权限，请在 macOS 系统设置中为终端/运行器授予“辅助功能（Accessibility）”权限。

12. 信息源接入（自然语言 + 加入按钮）：
- Dashboard 提供“信息源接入（自然语言）”卡片，显示 Slack/Gmail/Google Calendar 当前状态与 webhook 地址。
- 点击每个 Provider 的“加入”按钮会自动填充自然语言模板，你只需补齐 secret/token/topic/channel ids。
- 提交后会写入运行时配置并立即生效（同时持久化到 `config/integrations.runtime.json`）。
...
## Skill / Connector 接口规范（增量 v0.3）

Steward 将外部 skill/MCP 统一抽象为 Connector，不直接绕过执行层策略。

1. `capabilities`：声明可读写能力与支持动作。
2. `required_scopes`：声明最小权限范围。
3. `read_only_mode`：默认 `true`，逐步放开写权限。
4. `pull(cursor)`：增量拉取标准事件。
5. `execute(action)`：执行动作并返回可验证结果。
6. `health()`：返回连接状态与错误码。

## Skill 与 MCP 配置（增量 v0.3）

Steward 支持用户按策略配置 Skill 与 MCP，并统一接入执行层门禁。

1. 启用方式：通过配置文件声明 `skills.allowlist` 与 `mcp_servers`。
2. 安全默认：新接入能力默认 `read_only`，逐步放开写权限。
3. 统一治理：无论来自 Skill 还是 MCP，动作都必须经过 `Policy Gate`。
4. 运行控制：支持按连接器启用/禁用、健康检查失败自动降级。
5. 审计一致：所有外部能力执行都写入统一 `DecisionLog`。

## Skill / MCP 自管理能力（增量 v0.4）

Steward 具备“按需安装、升级、回滚” Skill 与 MCP 的能力，用于扩展感知与执行范围。

1. 能力发现：根据当前任务缺口识别所需能力（例如缺少邮件连接器）。
2. 授权后安装：从受信任来源拉取并安装指定 Skill/MCP（需用户确认）。
3. 自动升级：按策略检查新版本并灰度升级。
4. 健康回滚：升级失败或健康检查异常时自动回滚到上一个稳定版本。
5. 生命周期管理：支持启用、禁用、卸载与版本锁定。
6. 安装建议优先：发现能力缺口后，默认进入定时简报建议（例如“缺少 Jira 连接器，是否一键授权安装？”），由用户决定是否执行。

### 安装提议与打扰预算（增量 v0.4）

- 不绕过预算：能力安装/授权提议不得绕过打扰预算策略。
- 默认渠道：安装提议默认进入定时简报，不触发实时打断。
- 权力归还用户：安装与授权动作必须用户明确确认后执行。

### 自管理安全门禁（增量 v0.4）

- 来源白名单：仅允许受信任 registry / repo。
- 完整性校验：安装包签名或校验和验证通过后才可安装。
- 权限审查：新能力默认只读；申请写权限需命中策略门禁。
- 沙箱隔离：外部能力在受限运行环境执行，限制文件与网络访问。
- 审计追踪：记录安装来源、版本、权限变化、执行结果。
